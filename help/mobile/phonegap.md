...
1. Marketo PhoneGap 플러그인을 설정합니다.

   PhoneGap 애플리케이션 디렉터리로 이동하고 다음 명령을 실행하여 Marketo 플러그인을 추가합니다.
...
1. FCM 플러그인을 설치합니다.
   ...
   다음 명령을 실행하여 플러그인이 추가되었는지 확인합니다.
...
Cordova Android 플랫폼을 빌드한 후 Android Studio에서 앱을 엽니다. `com.marketo.plugin` 폴더의 `Marketo.gradle` 파일에서 `dirs` 값을 업데이트합니다.
...
앱에 대한 대상 플랫폼 추가: `$cordova platform add android` `$ cordova platform add ios`

추가된 플랫폼 확인: `$cordova platform ls`
...
1. Firebase 콘솔에서 Firebase 앱을 구성합니다.
   ...
   1. [!UICONTROL Project Overview]의 **[!UICONTROL Project Settings]**(으)로 이동합니다.
      1. **[!UICONTROL General]** 탭을 선택하고 &#39;google-services.json&#39; 파일을 다운로드합니다.
      1. **[!UICONTROL Cloud Messaging]** 탭을 선택합니다. [!UICONTROL Server Key] 및 [!UICONTROL Sender ID]을(를) 복사하여 Marketo에 제공합니다.
   1. PhoneGap 앱에서 FCM을 구성합니다.
      1. 다운로드한 &#39;google-services.json&#39; 파일을 PhoneGap 앱 모듈 루트 디렉터리로 이동합니다.
...
PhoneGap 앱의 프레임워크 유형으로 `phonegap`을(를) 전달합니다.
...
- GCM_PROJECT_ID: 앱을 만든 후 [Google 개발자 콘솔](https://console.developers.google.com/)에서 찾은 GCM 프로젝트 ID입니다.
