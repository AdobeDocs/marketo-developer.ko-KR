...
`Note:` 벌크 API 끝점에는 다른 끝점처럼 &#39;/rest&#39; 접두사가 붙지 않습니다.

## 인증

벌크 추출 API는 다른 Marketo REST API와 동일한 OAuth 2.0 인증 방법을 사용합니다. 이를 위해서는 쿼리-문자열 매개 변수로 포함할 유효한 액세스 토큰이 필요합니다 `access_token={_AccessToken_}`또는 HTTP 헤더로서 `Authorization: Bearer {_AccessToken_}`.
...
- 파일 보존 기간: 7일
- 기본 일별 내보내기 할당: 500MB(매일 오전 12:00CST 시 재설정됨) 구매 가능한 증가.
...
대량 추출 API는 대량 추출 작업에 의해 검색된 데이터의 디스크 크기를 기반으로 측정됩니다. 작업의 명시적 크기(바이트)는 `fileSize` 내보내기 작업의 완료된 상태 응답의 속성입니다.

일일 할당량은 일일 최대 500MB이며 리드, 활동, 프로그램 멤버 및 사용자 정의 객체 간에 공유됩니다. 할당량이 초과되면 일일 할당량이 자정에 재설정될 때까지 다른 작업을 만들거나 대기열에 넣을 수 없습니다 [중부 표준시](https://en.wikipedia.org/wiki/Central_Time_Zone). 그때까지 &quot;1029, 일일 내보내기 할당량 초과&quot; 오류가 반환됩니다. 일일 할당량 외에 최대 파일 크기는 없습니다.
...
리드/활동/프로그램 멤버 작업 내보내기 가져오기 를 호출하여 현재 날짜에 사용된 총 디스크 양을 확인할 수 있습니다. 이러한 엔드포인트는 지난 7일 동안의 모든 작업 목록을 반환합니다. 해당 목록을 현재 일에 완료된 작업으로만 필터링할 수 있습니다( 사용). `status` 및 `finishedAt` 속성)을 참조하십시오. 그런 다음 해당 작업에 대한 파일 크기를 합하여 총 사용량을 생성합니다. 파일을 삭제하여 디스크 공간을 다시 확보할 수 있는 방법은 없습니다.
...
작업을 생성하면에 작업 ID가 반환됩니다 `exportId` 특성. 그런 다음 이 작업 ID를 사용하여 작업을 대기열에 넣거나, 취소하거나, 상태를 확인하거나, 완료된 파일을 검색할 수 있습니다.
...
경우에 따라 최근 작업을 검색해야 할 수 있습니다. 해당 객체 유형에 대한 내보내기 작업 가져오기를 사용하여 쉽게 수행할 수 있습니다. 각 내보내기 작업 가져오기 끝점은 `status` 필터 필드, a  `batchSize` 반환되는 작업 수를 제한하려면 `nextPageToken` 큰 결과 집합을 통해 페이징할 수 있습니다. 상태 필터는 내보내기 작업에 대해 생성됨, 큐에 있음, 처리, 취소됨, 완료됨 및 실패의 각 유효한 상태를 지원합니다. batchSize의 최대 및 기본값은 300입니다. 리드 내보내기 작업 목록을 가져오겠습니다.
...
끝점이 다음으로 응답함 `status` 결과 배열의 해당 오브젝트 유형에 대해 지난 7일 동안 생성된 각 작업의 응답입니다. 응답에는 호출을 수행하는 API 사용자가 소유한 작업에 대한 결과만 포함됩니다.
...
더 이너 `status` 멤버는 작업의 진행 상황을 나타내며 생성됨, 대기열에 있음, 처리, 취소됨, 완료됨, 실패함 값 중 하나일 수 있습니다. 이 경우 작업이 완료되었으므로 폴링을 중단하고 계속 파일을 검색할 수 있습니다. 완료되면 `fileSize` member는 파일의 총 길이(바이트)를 나타내며, `fileChecksum` 멤버에 파일의 SHA-256 해시가 들어 있습니다. 작업 상태는 완료됨 또는 실패 상태에 도달한 후 30일 동안 사용할 수 있습니다.
...
추출된 데이터의 부분적 및 재시작 친화적 검색을 지원하기 위해 파일 엔드포인트는 선택적으로 HTTP 헤더를 지원합니다 `Range` 유형 `bytes` (마다 [RFC 7233](https://datatracker.ietf.org/doc/html/rfc7233)). 헤더가 설정되지 않은 경우 전체 콘텐츠가 반환됩니다. 파일의 처음 10,000바이트를 검색하려면 바이트 0부터 다음 헤더를 GET 요청의 일부로 엔드포인트에 전달합니다.
...
파일을 부분적으로 검색하거나 나중에 를 사용하여 다시 시작할 수 있습니다 `Range` 머리글입니다. 파일의 범위는 바이트 0에서 시작하여 다음 값으로 끝납니다. `fileSize` 빼기 1. 파일 길이도 값 의 분모로 보고됩니다. `Content-Range` 내보내기 파일 가져오기 끝점을 호출할 때의 응답 헤더입니다. 읽어들이기가 부분적으로 실패하면 나중에 다시 시작할 수 있습니다. 예를 들어 1000바이트 길이의 파일을 검색하려고 하지만 처음 725바이트만 받은 경우 끝점을 다시 호출하고 새 범위를 전달하여 실패 지점에서 검색을 재시도할 수 있습니다.
...
작업 상태 끝점이 체크섬을 `fileChecksum` 속성 시점 `status` 은(는) &quot;완료됨&quot;입니다. 체크섬은 내보낸 파일의 SHA-256 해시입니다. 검색된 파일의 SHA-256 해시와 체크섬을 비교하여 파일이 완료되었는지 확인할 수 있습니다.
