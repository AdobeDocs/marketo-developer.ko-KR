...
리드는 다양한 필터 옵션을 지원합니다. 특정 필터(다음 포함) `updatedAt`, `smartListName`, 및 `smartListId` 아직 모든 구독으로 롤아웃되지 않은 추가 인프라 구성 요소가 필요합니다. 내보내기 작업당 하나의 필터 유형만 지정할 수 있습니다.

| 필터 유형 | 데이터 유형 | 참고 사항 |
|---|---|---|
| createdAt | 날짜 범위 | 멤버가 있는 JSON 개체 수락 `startAt` 및 `endAt`. `startAt` 에서는 로우 워터마크를 나타내는 날짜/시간을 수락하고 `endAt` 하이 워터마크를 나타내는 날짜/시간을 허용합니다. 범위는 31일 이하여야 합니다. 날짜/시간은 밀리초 없이 ISO-8601 형식이어야 합니다. 이 필터 유형의 작업은 날짜 범위 내에서 만든 액세스 가능한 모든 레코드를 반환합니다. |
| updatedAt* | 날짜 범위 | 멤버가 있는 JSON 개체 수락 `startAt` 및 `endAt`. `startAt` 에서는 로우 워터마크를 나타내는 날짜/시간을 수락하고 `endAt` 하이 워터마크를 나타내는 날짜/시간을 허용합니다. 범위는 31일 이하여야 합니다. 날짜/시간은 밀리초 없이 ISO-8601 형식이어야 합니다. 참고: 이 필터는 표준 필드에 대한 업데이트만 반영하는 표시되는 &quot;updatedAt&quot; 필드를 필터링하지 않습니다. 이 필터는 잠재 고객 레코드에 대한 가장 최근 필드 업데이트가 이루어진 시점을 기준으로 필터링합니다.이 필터 유형을 가진 작업은 날짜 범위 내에서 가장 최근 업데이트된 액세스 가능한 모든 레코드를 반환합니다. |
...
작업에 대한 매개 변수는 를 사용하여 내보내기를 시작하기 전에 정의합니다. [내보내기 리드 작업 만들기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/createExportLeadsUsingPOST) 엔드포인트. 다음을 정의해야 합니다. `fields` 내보내기에 필요한 매개 변수의 유형 `filter`, `format` 및 열 머리글 이름(있는 경우)입니다.
...
이 요청은 해당 값을 포함하여 2017년 1월 1일부터 2017년 1월 31일 사이에 생성된 리드 세트를 내보내기 시작합니다 `firstName`, `lastName`, `id`, 및 `email` 필드.
...
작업이 생성되었음을 나타내는 상태 응답이 반환됩니다. 작업이 정의되고 생성되었지만 아직 시작되지 않았습니다. 이렇게 하려면 [리드 내보내기 작업 큐에 넣기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/enqueueExportLeadsUsingPOST) 만들기 상태 응답의 exportId를 사용하여 끝점을 호출해야 합니다.
...
다음으로 응답함: `status` 사용 가능한 내보내기 슬롯이 있는 경우 &quot;대기 중&quot; 이후 &quot;처리 중&quot;으로 설정됩니다.

## 폴링 작업 상태

`Note:` 동일한 API 사용자가 만든 작업에 대해서만 상태를 검색할 수 있습니다.

비동기 끝점이므로 작업을 만든 후 상태를 폴링하여 진행률을 확인해야 합니다. 을(를) 사용하여 투표 [리드 내보내기 작업 상태 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/getExportLeadsStatusUsingGET) 엔드포인트. 상태는 60초마다 한 번만 업데이트되므로 이보다 낮은 폴링 빈도는 권장되지 않으며 거의 모든 경우에 여전히 과도합니다. 투표를 간단히 살펴봅시다.
...
완료된 리드 내보내기의 파일을 검색하려면 [내보내기 리드 파일 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/getExportLeadsFileUsingGET) 엔드포인트 `exportId`.
...
요청한 리드 필드가 비어 있는 경우(데이터 없음) `null` 는 내보내기 파일의 해당 필드에 배치됩니다. 아래 예에서는 반환된 리드에 대한 이메일 필드가 비어 있습니다.
...
추출된 데이터의 부분 검색 및 재시작 친화성을 지원하기 위해 파일 엔드포인트는 선택적으로 바이트 유형의 HTTP 헤더 범위를 지원합니다. 헤더가 설정되지 않은 경우 전체 콘텐츠가 반환됩니다. Marketo에서 범위 헤더 사용에 대한 자세한 내용 [일괄 추출](bulk-extract.md).

## 작업 취소

작업이 잘못 구성되었거나 필요하지 않은 경우 를 사용하여 쉽게 취소할 수 있습니다. [리드 내보내기 작업 취소](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/cancelExportLeadsUsingPOST) 끝점:
