...
Lead API 의 주요 기능 중 하나는 Describe 메서드입니다. 리드 설명을 사용하여 REST API 및 SOAP API와 각 REST API에 대한 메타데이터를 모두 통해 상호 작용에 사용할 수 있는 필드의 전체 목록을 검색합니다.
...
일반적으로 응답에는 결과 배열에 훨씬 큰 필드 집합이 포함되지만 데모용으로 생략됩니다. 결과 배열의 각 항목은 리드 레코드에서 사용할 수 있는 필드에 해당하며 최소한 ID, displayName 및 데이터 형식을 갖게 됩니다. rest 및 soap 자식 개체는 주어진 필드에 대해 존재할 수도 있고 존재하지 않을 수도 있으며, 그 존재는 필드가 REST 또는 SOAP API에서 사용하기에 유효한지 여부를 나타냅니다. 다음 `readOnly` 속성은 해당 API(REST 또는 SOAP)를 통해 필드가 읽기 전용인지 여부를 나타냅니다. length 속성은 필드의 최대 길이(있는 경우)를 나타냅니다. dataType 속성은 필드의 데이터 형식을 나타냅니다.

## 쿼리

리드 검색에는 ID별 리드 가져오기 및 필터 유형별 리드 가져오기 두 가지 기본 방법이 있습니다. Id별 Lead 가져오기 는 단일 Lead ID 를 경로 매개 변수로 사용하고 단일 Lead 레코드를 반환합니다.

선택적으로 반환할 필드 이름의 쉼표로 구분된 목록이 포함된 필드 매개 변수를 전달할 수 있습니다. 필드 매개 변수가 이 요청에 포함되지 않으면 다음 기본 필드가 반환됩니다. `email`, `updatedAt`, `createdAt`, `lastName`, `firstName`, 및 `id`. 필드 목록을 요청할 때 특정 필드가 요청되었지만 반환되지 않은 경우 값이 null로 간주됩니다.
...
필터 유형별 리드 가져오기는 동일한 유형의 레코드를 반환하지만, 페이지당 최대 300개를 반환할 수 있습니다. 이를 위해서는 다음이 필요합니다. `filterType` 및 `filterValues` 쿼리 매개 변수.

`filterType` 는 모든 사용자 정의 필드 또는 일반적으로 사용되는 대부분의 필드를 허용합니다. 호출 `Describe2` 에서 사용할 수 있는 검색 가능한 필드의 포괄적인 목록을 가져올 엔드포인트 `filterType`. 사용자 지정 필드로 검색할 때 다음 데이터 유형만 지원됩니다. `string`, `email`, `integer`. 필드 세부 사항(설명, 유형 등)을 가져올 수 있습니다. 위에서 설명한 방법을 사용합니다.

`filterValues` 은 쉼표로 구분된 형식으로 최대 300개의 값을 허용합니다. 이 호출은 잠재 고객의 필드가 포함된 필드 중 하나와 일치하는 레코드를 검색합니다 `filterValues`. 리드 필터와 일치하는 리드 수가 1,000보다 크면 &quot;1003, 필터와 일치하는 결과가 너무 많습니다.&quot;라는 오류가 반환됩니다.
...
이 호출은에 포함된 ID와 일치하는 레코드를 검색합니다 `filterValues`를 반환하고, 일치하는 레코드를 모두 반환합니다.
...
ID로 리드 가져오기 및 필터 유형별로 리드 가져오기 는 모두 쉼표로 구분된 API 필드 목록을 수락하는 필드 쿼리 매개 변수도 수락합니다. 이 필드가 포함된 경우 응답의 각 레코드에는 나열된 필드가 포함됩니다.  생략하면 기본 필드 세트가 반환됩니다. `id`, `email`, `updatedAt`, `createdAt`, `firstName`, 및 `lastName`.
...
이 요청에는 두 개의 중요한 필드가 표시됩니다. `action` 및 `lookupField`.  `action` 요청의 작업 유형을 지정하며, 다음과 같을 수 있습니다. `createOrUpdate`, `createOnly`, `updateOnly`, 또는 `createDuplicate`. 생략하면 작업의 기본값이 로 설정됩니다. `createOrUpdate`.  다음 `lookupField` 매개 변수는 작업이 다음 중 하나일 때 사용할 키를 지정합니다. `createOrUpdate` 또는 `updateOnly`. If `lookupField` 가 생략되어 있고 기본 키가 `email`.

기본적으로 기본 파티션이 사용됩니다. 선택적으로 다음을 지정할 수 있습니다. `partitionName` 매개 변수 - 작업이 인 경우에만 작동합니다. `createOnly` 또는 `createOrUpdate`. 대상 `partitionName` 추가 중복 제거 기준으로 작동하려면 사용자 정의 중복 제거 규칙에서 소스 유형의 일부여야 합니다. 업데이트 작업 중에 지정된 파티션에 잠재 고객이 없으면 오류가 반환됩니다. API 전용 사용자에게 지정된 파티션에 액세스할 수 있는 권한이 없는 경우 오류가 반환됩니다.

다음 `id` 필드는 을 사용할 때만 매개 변수로 포함될 수 있습니다. `updateOnly` 작업, 다음으로 `id` 는 시스템 관리 고유 키입니다.

요청에도 다음이 있어야 합니다. `input` 매개 변수 : 잠재 고객 레코드 배열입니다. 각 리드 레코드는 리드 필드가 수에 관계없이 있는 JSON 개체입니다. 레코드에 포함된 키는 해당 레코드에 대해 고유해야 하며 모든 JSON 문자열은 UTF-8로 인코딩되어야 합니다. 다음 `externalCompanyId` 리드 레코드를 회사 레코드에 연결하는 데 필드를 사용할 수 있습니다. 다음 `externalSalesPersonId` 필드는 잠재 고객 레코드를 영업 사원 레코드에 연결하는 데 사용할 수 있습니다.

주: 가망 고객 갱신 요청을 동시에 또는 빠르게 연속하여 수행할 때, 동일한 값을 가진 후속 호출이 첫 번째 반환 전에 수행되면 동일한 키 값으로 여러 요청을 수행할 때 중복 레코드가 발생할 수 있습니다. 다음 중 하나를 사용하면 이러한 문제를 방지할 수 있습니다. `createOnly`, 또는 `updateOnly` 필요에 따라 또는 호출을 큐에 넣고 동일한 키로 후속 업데이트를 호출하기 전에 호출이 반환되기를 기다리는 작업을 수행할 수 있습니다.
...
리드 필드 가져오기 끝점은 를 포함하여 리드 오브젝트의 모든 필드에 대한 메타데이터를 검색합니다. 기본적으로 최대 300개의 레코드가 반환됩니다. 다음을 사용할 수 있습니다. `batchSize` 쿼리 매개 변수를 사용하여 이 숫자를 줄이십시오. 다음과 같은 경우 `moreResult` 속성이 true이면 더 많은 결과를 사용할 수 있음을 의미합니다. 다음 시간까지 이 엔드포인트를 계속 호출합니다. `moreResult` 속성은 false를 반환하며, 이것은 사용 가능한 결과가 없음을 의미합니다. 다음 `nextPageToken` 이 API에서 반환된 내용은 항상 이 호출의 다음 반복에 재사용되어야 합니다.
...
필수 입력 매개 변수는 리드 필드 개체의 배열입니다. 각 객체에는 하나 이상의 속성이 포함됩니다. 필수 속성은 다음과 같습니다. `displayName`, `name`, 및 `dataType` 필드의 UI 표시 이름, 필드의 API 이름 및 필드 유형에 각각 해당합니다.  선택적으로 다음을 지정할 수 있습니다. `description`, `isHidden`, `isHtmlEncodingInEmail`, 및 `isSensitive`.

이름 및 와 관련된 몇 가지 규칙이 있습니다 `displayName` 이름 지정. 이름 속성은 고유해야 하며 문자로 시작하고 문자, 숫자 또는 밑줄만 포함해야 합니다. 다음 `displayName` 은(는) 고유해야 하며 특수 문자를 포함할 수 없습니다.  일반적인 명명 규칙은 다음과 같은 카멜 대/소문자를 적용하는 것입니다 `displayName` 을 입력하여 이름을 생성합니다. 예: `displayName` 의 &quot;My Custom Field&quot;는 &quot;myCustomField&quot;의 이름을 생성합니다.
...
필수 `fieldApiName` path 매개 변수는 업데이트할 필드의 API 이름을 지정합니다. 필수 입력 매개 변수는 단일 리드 필드 개체를 포함하는 배열입니다.  필드 개체에는 하나 이상의 특성이 포함되어 있습니다.
...
푸시 리드는 Marketo에 리드 동기화를 위한 대안으로서, 기본적으로 표준 동기화 리드(Marketo 양식과 사용 방식 유사)보다 더 많은 트리거 기능을 허용하도록 설계되었습니다. 리드 필드의 동기화 외에도 이 엔드포인트는 엔드포인트에 전달되는 쿠키 값을 기반으로 리드 연결을 허용합니다. 다음을 전달하여 이 작업을 수행합니다. `mkt_tok` Marketo 이메일을 클릭하거나 호출에서 프로그램 이름을 전달하여 생성된 값입니다. 이 끝점은 또한 Marketo의 프로그램 및/또는 캠페인에 연결된 단일 트리거 가능 활동을 만듭니다. 이를 통해 특정 캠페인 또는 프로그램에 기인한 리드 캡처 이벤트를 트리거하여 Marketo 내에서 관련 워크플로우를 시작할 수 있습니다.

푸시 리드 인터페이스는 동기화 리드와 매우 유사합니다. 동일한 기본 키가 모두 유효하며 필드에 동일한 API 이름이 사용됩니다(항상 업데이트 작업이므로 작업 매개 변수는 없음). 다음 `programName` 및 입력 매개 변수가 필요하고 `lookupField`, `source`, 및 `reason` 매개 변수는 선택 사항입니다. 입력 매개변수는 리드 객체의 배열입니다. 결과 활동은 이름이 지정된 해당 프로그램에 기인합니다. 다음 `source` 및 `reason` 매개 변수는 요청에 추가하여 결과 활동에 해당 값을 포함할 수 있는 임의의 문자열 필드입니다. 해당 트리거(리드가 Marketo으로 푸시됨) 및 필터(리드가 Marketo으로 푸시됨)의 제약 조건으로 사용할 수 있습니다.
...
을(를) 전달하려면 `mkt_tok` 매개 변수에서 입력 매개 변수의 잠재 고객 레코드 내에 있는 mktToken 멤버에 값을 다음과 같이 할당합니다.
...
양식을 제출하는 것은 표준 리드 데이터베이스 패턴을 따릅니다. 단일 오브젝트 레코드가 POST 요청의 JSON 본문의 필수 입력 멤버에 전달됩니다. 필수 `formId` 멤버에는 대상 Marketo 양식 ID가 포함되어 있습니다.

선택 사항 `programId` 리드를 추가할 프로그램을 지정하거나 프로그램 멤버 사용자 정의 필드를 추가할 프로그램을 지정하는 데 사용할 수 있습니다. If `programId` 이 제공되면, 리드가 프로그램에 추가되고 양식에 포함된 모든 프로그램 멤버 필드도 추가됩니다. 지정된 프로그램은 양식과 동일한 작업 영역에 있어야 합니다. 양식에 프로그램 멤버 사용자 정의 필드가 포함되지 않은 경우 `programId` 가 제공되지 않으면 리드가 프로그램에 추가되지 않습니다. 양식이 프로그램에 있고 `programId` 이 프로그램은 제공되지 않으며 하나 이상의 프로그램 멤버 사용자 정의 필드가 양식에 있는 경우 사용됩니다.

입력 레코드 내에서 `leadFormFields` 개체가 필요합니다. 이 개체에는 채울 양식 필드에 해당하는 이름/값 쌍이 하나 이상 포함되어 있습니다.  지정된 모든 필드는 지정된 양식 내에서 정의되어야 합니다. 이름은 필드에 대한 REST API 이름입니다. 다음 사항에 주의하십시오. `email` 필수 필드입니다.

다음 `visitorData` member 객체는 선택 사항이며 다음을 포함하는 페이지 방문 데이터에 해당하는 이름/값 쌍을 포함합니다. `pageURL`, `queryString`, `leadClientIpAddress`, 및 `userAgentString`. 필터링 및 트리거를 위해 추가 활동 필드를 채우는 데 사용할 수 있습니다.

쿠키 멤버 문자열은 선택 사항이며 Marketo에서 Munchkin 쿠키를 사용자 레코드와 연결할 수 있습니다. 새 잠재 고객을 만들 때 쿠키 값이 이전에 알려진 다른 레코드와 연결되어 있지 않은 경우 이전의 모든 익명 활동은 해당 잠재 고객과 연결됩니다. 쿠키 값이 이전에 연결된 경우 새 활동이 레코드에 대해 추적되지만 이전 활동은 알려진 기존 레코드에서 마이그레이션되지 않습니다. 활동 내역이 없는 새 잠재 고객을 만들려면 쿠키 구성원을 생략하면 됩니다.
...
경우에 따라 중복 레코드를 병합해야 하며 Marketo은 리드 병합 API를 통해 이를 용이하게 합니다. 리드를 병합하면 활동 로그, 프로그램, 캠페인, 목록 멤버십 및 CRM 정보가 결합되고 모든 필드 값이 단일 레코드로 병합됩니다. 리드 병합은 리드 ID를 경로 매개 변수로 가져오고 `leadId` 를 쿼리 매개 변수 또는 의 쉼표로 구분된 id 목록으로 `leadIds` 매개 변수.
...
경로 매개 변수에 지정된 리드는 우승 리드이므로 병합되는 레코드 사이에 충돌하는 필드가 있으면 우승 레코드의 필드가 비어 있고 손실 레코드의 해당 필드가 아닌 경우를 제외하고 우승자의 값이 사용됩니다. 다음 중 하나에 지정된 리드 `leadId` 또는 `leadIds` 매개 변수는 손실 잠재 고객입니다.

SFDC-sync가 활성화된 구독이 있는 경우 다음을 사용할 수도 있습니다. `mergeInCRM` 매개 변수를 채우는 방법을 설명합니다. true로 설정하면 CRM의 해당 병합도 수행됩니다. 두 리드가 모두 SFDC에 있고 한 리드가 CRM 리드이고 다른 리드가 CRM 연락처인 경우 승자는 CRM 연락처입니다(어떤 리드가 승자로 지정되었는지에 관계없이). 잠재 고객 중 하나가 SFDC에 있고 다른 하나는 Marketo에만 있는 경우 우승자는 SFDC 잠재 고객입니다(우승자로 지정된 잠재 고객에 관계없이).

## 웹 활동 연결

Marketo은 리드 추적(Munchkin)을 통해 웹 사이트 방문자 및 Marketo 랜딩 페이지에 대한 웹 활동을 기록합니다. 방문 및 클릭이라는 이러한 활동은 잠재 고객의 브라우저에 설정된 &quot;_mkto_trk&quot; 쿠키에 해당하는 키로 기록되며, Marketo은 이 키를 사용하여 동일한 사용자의 활동을 추적합니다. 일반적으로 잠재 고객 레코드와 연결은 잠재 고객이 Marketo 이메일을 통해 클릭스루하거나 Marketo 양식을 작성할 때 발생하지만, 경우에 따라 다른 유형의 이벤트에 의해 연결이 트리거될 수 있으며 연결된 잠재 고객 끝점을 사용하여 이 작업을 수행할 수 있습니다. 끝점은 알려진 잠재 고객 레코드의 ID를 경로 매개 변수로 사용하고 쿠키 쿼리 매개 변수의 &quot;_mkto_trk&quot; 쿠키 값을 사용합니다.
...
Marketo UI를 통해 목록 ID에 액세스하려면 목록으로 이동합니다. 목록 `id` 정적 목록의 URL에 있습니다. `https://app-****.marketo.com/#ST1001A1`. 이 예에서 1001은 `id` 목록에 대해.
...
리드 ID 끝점별 목록 가져오기는 리드 레코드를 가져옵니다. `id` path 매개 변수를 반환하고 잠재 고객이 멤버인 모든 정적 목록 레코드를 반환합니다.
...
프로그램 멤버십은 목록과 유사한 방식으로 검색할 수 있습니다. 프로그램 ID 끝점별 리드 가져오기 를 호출하고 를 전달할 때 동일한 선택적 요청 매개 변수를 사용할 수 있습니다. `programId` 경로 매개 변수.

선택적으로 반환할 필드 이름의 쉼표로 구분된 목록이 포함된 필드 매개 변수를 전달할 수 있습니다. 필드 매개 변수가 이 요청에 포함되지 않으면 다음 기본 필드가 반환됩니다. `email`, `updatedAt`, `createdAt`, `lastName`, `firstName`, `membership`, 및 `id`. 필드 목록을 요청할 때 특정 필드가 요청되었지만 반환되지 않은 경우 값이 null로 간주됩니다.

각 레코드에 &quot;멤버십&quot;이라는 하위 개체도 있다는 점을 제외하고 결과 배열의 각 항목이 리드이므로 응답 구조는 매우 유사합니다. 이 멤버십 개체에는 호출에 표시된 프로그램에 대한 잠재 고객의 관계에 대한 데이터가 포함되며 항상 해당 데이터가 표시됩니다 `progressionStatus`, `acquiredBy`, `reachedSuccess`, 및 `membershipDate`. 상위 프로그램도 참여 프로그램인 경우 멤버십에 회원이 포함됩니다. `stream`, `nurtureCadence`, 및 `isExhausted` 참여 프로그램에서의 위치 및 활동을 나타냅니다.
...
리드 ID별 프로그램 가져오기 엔드포인트는 리드 레코드 ID 경로 매개 변수를 사용하고 리드가 멤버로 있는 모든 프로그램 레코드를 반환합니다. 선택 사항 `filterType` 및 `filterValues` 매개 변수를 사용하면 프로그램 ID를 필터링할 수 있습니다.
