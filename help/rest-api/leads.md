...
Lead API 의 주요 기능 중 하나는 Describe 메서드입니다. 리드 설명을 사용하여 REST API 및 SOAP API와 각 API에 대한 메타데이터를 모두 통해 상호 작용에 사용할 수 있는 필드의 전체 목록을 검색합니다.
...
SFDC-sync를 사용하도록 설정한 구독이 있는 경우 요청에서 `mergeInCRM` 매개 변수를 사용할 수도 있습니다. true로 설정하면 CRM의 해당 병합도 수행됩니다. 두 리드가 모두 SFDC에 있고 한 리드가 CRM 리드이고 다른 리드가 CRM 연락처인 경우 승자는 CRM 연락처입니다(어떤 리드가 승자로 지정되었는지에 관계없이). 잠재 고객 중 하나가 SFDC에 있고 다른 하나는 Marketo에만 있는 경우 우승자는 SFDC 잠재 고객입니다(우승자로 지정된 잠재 고객에 관계없이).
