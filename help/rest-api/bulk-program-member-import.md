...
대량의 프로그램 멤버 레코드의 경우 프로그램 멤버를 [벌크 API](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Import-Program-Members). 이렇게 하면 구분 기호(쉼표, 탭 또는 세미콜론)가 있는 플랫 파일을 사용하여 레코드 목록을 Marketo에 가져올 수 있습니다. 파일 합계가 10MB 미만인 한 파일에는 여러 개의 레코드가 포함될 수 있습니다. 레코드 작업은 &quot;삽입 또는 업데이트&quot;만 가능합니다.
...
파일의 첫 번째 행은 각 행의 값을 매핑할 필드로 해당 REST API 이름을 나열하는 헤더여야 합니다. REST API 이름은 [리드 설명](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Leads/operation/describeUsingGET_2) 및/또는 [프로그램 구성원 설명](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Leads/operation/describeProgramMemberUsingGET) 엔드포인트. 레코드에는 리드 필드, 사용자 정의 리드 필드 및 사용자 정의 프로그램 멤버 필드가 포함될 수 있습니다.
...
다음 [프로그램 구성원 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Import-Program-Members/operation/importProgramMemberUsingPOST) endpoint는 프로그램 멤버 레코드가 포함된 파일을 읽고 지정된 상태의 프로그램에 추가합니다. 레코드에는 리드 필드와 프로그램 멤버 사용자 정의 필드가 모두 포함될 수 있습니다. 모든 레코드에는 중복 제거를 위해 사용되는 이메일 필드가 포함되어야 합니다.

다음 `programId` path 매개 변수는 멤버가 추가되는 프로그램을 지정합니다.

세 가지 필수 쿼리 매개 변수가 있습니다. 다음 `format` 매개 변수는 가져오기 파일 형식(CSV, TSV 또는 SSV)을 지정합니다. `programMemberStatus` 매개 변수는 프로그램에 추가되는 멤버의 프로그램 상태를 지정합니다. `file` 매개 변수에는 프로그램 멤버 레코드가 포함된 가져오기 파일의 이름이 포함됩니다.
...
호출에 대한 응답으로 다음 항목이 있습니다. `batchId` 및 a `status` 결과 배열의 레코드에 대한 필드. 이 끝점은 비동기적이므로 큐에 있음, 가져오기 또는 실패 상태를 반환할 수 있습니다. 다음을 유지해야 합니다. `batchId` 가져오기 작업의 상태를 가져오고, 완료 시 실패 및/또는 경고를 검색합니다. 다음 `batchId` 7일 동안 유효합니다.
...
가져오기 작업이 생성되면 해당 상태를 쿼리해야 합니다. 가져오기 작업을 5-30초마다 폴링하는 것이 가장 좋습니다. 다음을 전달하여 이 작업을 수행합니다. `batchId` 경로 매개 변수 [가져오기 프로그램 멤버 상태 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Import-Program-Members/operation/getImportProgramMemberStatusUsingGET) 엔드포인트.
...
실패는 `numOfRowsFailed` 의 속성 [가져오기 프로그램 멤버 상태 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Import-Program-Members/operation/getImportProgramMemberStatusUsingGET) 응답. numOfRowsFailed가 0보다 크면 해당 값은 발생한 실패 횟수를 나타냅니다.

사용 [가져오기 프로그램 멤버 가져오기 실패](http://TODO) 를 전달하여 실패한 행의 원인 및 레코드를 검색하는 엔드포인트 `batchId` 경로 매개 변수.

```
GET /bulk/v1/program/members/import/{batchId}/failures.json
```

끝점은 레코드가 실패한 이유를 나타내는 메시지와 함께 실패한 행을 나타내는 파일로 응답합니다. 파일의 형식은에 지정된 것과 같습니다. `format` 작업 생성 중 매개 변수. 각 레코드에 오류에 대한 설명과 함께 추가 필드가 추가됩니다.
...
작업 상태를 확인하면 다음과 같이 표시됩니다. `numOfRowsFailed` 은(는) 실패가 발생했음을 나타내는 1입니다.
...
경고는 `numOfRowsWithWarning` 의 속성 [가져오기 프로그램 멤버 상태 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Import-Program-Members/operation/getImportProgramMemberStatusUsingGET) 응답. If `numOfRowsWithWarning` 이(가) 0보다 큰 경우 해당 값은 발생한 경고 수를 나타냅니다.

사용 [가져오기 프로그램 멤버 경고 가져오기](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Import-Program-Members/operation/getImportProgramMemberWarningsUsingGET) 를 전달하여 레코드 및 경고 행 원인을 검색하는 엔드포인트 `batchId` 경로 매개 변수.

```
GET /bulk/v1/program/members/import/{batchId}/warnings.json
```

끝점은 레코드에서 경고를 생성한 이유를 나타내는 메시지와 함께 경고를 생성한 행을 나타내는 파일로 응답합니다. 파일의 형식은에 지정된 것과 같습니다. `format` 작업 생성 중 매개 변수. 각 레코드에 경고에 대한 설명과 함께 추가 필드가 추가됩니다.
...
작업 상태를 확인하면 다음과 같이 표시됩니다. `numOfRowsWithWarning` 는 경고가 발생했음을 나타내는 1입니다.
