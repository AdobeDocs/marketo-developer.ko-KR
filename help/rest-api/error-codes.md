...
- **HTTP 수준:**(`4xx` 코드로 표시됨).
- **응답 수준:**&#x200B;이(가) JSON 응답의 &quot;오류&quot; 배열에 포함되었습니다.
...
일반적으로 클라이언트 ID 또는 클라이언트 암호가 잘못되었기 때문에 [ID](https://developer.adobe.com/marketo-apis/api/identity/) 끝점이 401 Unauthorized 오류를 반환할 수 있습니다. 다음 표에는 HTTP 수준 오류 코드가 나와 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>요청의 URI가 8k를 초과했습니다. URL에 매개 변수 '_method=GET'이 있고 요청 본문에 나머지 쿼리 문자열이 있는 POST로 요청을 다시 시도해야 합니다.</td>
    </tr>
  </tbody>
</table>
...
- `code`: 601에서 799까지의 인용 정수.
- `message`: 오류에 대한 일반 텍스트 이유입니다.
...
<table>
  ...
  <tr>
      ...
      <td>내부 서버 오류</td>
      <td>서버에서 예기치 않은 조건이 발생하여 요청을 수행할 수 없습니다.  Marketo 내에서, 여기에는 부적절한 형식의 REST API 요청 URL이 포함될 수 있습니다.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>동기화 리드 끝점에 대해서는 GET이 지원되지 않습니다. POST를 사용해야 합니다.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>일일 할당량에 도달함</td>
      <td>오늘 통화 수가 구독의 할당량을 초과했습니다(매일 오전 12시(CST)).&gt;할당량은 [관리]-&gt;[웹 서비스] 메뉴에서 찾을 수 있습니다. 계정 관리자를 통해 할당량을 늘릴 수 있습니다.</td>
    </tr>
    <tr>
