...
Forms은 표준 자산 검색 방법을 지원합니다. [id 기준](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/getLpFormByIdUsingGET), [이름순](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/getLpFormByNameUsingGET), 및 [검색 기준](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/browseForms2UsingGET). 각 양식 응답에는 필드 목록을 제외한 모든 속성이 포함됩니다.

### ID별

[ID로 양식 가져오기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/getLpFormByIdUsingGET) 양식 작성 `id` 를 경로 매개 변수로 사용하고 양식 레코드를 반환합니다.
...
[이름별 양식 가져오기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/getLpFormByNameUsingGET) 양식 작성 `name` 를 경로 매개 변수로 사용하고 양식 레코드를 반환합니다.
...
[Forms 가져오기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/browseForms2UsingGET) 양식은 다른 Asset API 검색 끝점과 마찬가지로 작동하며 선택적 필터링을 허용합니다. `status`, `maxReturn`, 및 `offset`. 상태는 승인됨, 승인됨, 초안 또는 초안일 수 있습니다.
...
다음 [다음에서 양식 사용:](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/getFormUsedByUsingGET) 끝점은 양식을 가져옵니다. `id` 를 경로 매개 변수로 사용하고, 양식에 종속된 에셋 목록을 반환합니다. Forms은 랜딩 페이지, 스마트 목록, 스마트 캠페인, 보고서, 이메일 프로그램 등의 자산 유형에서 사용할 수 있습니다.
...
날짜 [양식 만들기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/createLpFormsUsingPOST) 필수 필드에는 양식의 상위 폴더와 양식 이름만 있습니다. 다른 모든 매개 변수는 기본값과 함께 선택 사항입니다. 양식이 만들어지면 세 개의 기본 필드(이름, 성, 이메일)가 제공됩니다.
...
Forms은 [업데이트됨](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/updateFormsUsingPOST) id를 통해 유사한 호출을 생성합니다. 만들거나 업데이트하는 동안 기본 스타일 매개 변수에 액세스하고 이를 편집할 수 있으므로 양식을 최종 사용자에게 표시하는 방법을 수정할 수 있습니다.
...
Lead 필드의 경우 다음을 사용하여 수행됩니다. [사용 가능한 양식 필드 가져오기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/getAllFieldsUsingGET) 를 종단점으로 추가하고, 양식에 추가할 때 필드의 데이터 형식과 기본 메타데이터를 포함합니다.
...
프로그램 멤버 사용자 정의 필드의 경우 [사용 가능한 양식 프로그램 멤버 필드 가져오기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/getAllProgramMemberFieldsUsingGET)  프로그램 멤버 사용자 지정 필드 데이터 형식 및 기본 메타데이터를 검색할 종단점입니다. 폼에서 이러한 필드를 사용하려면 폼이 Design Studio가 아닌 프로그램 아래에 있어야 합니다. 이러한 필드를 사용하는 양식이 포함된 랜딩 페이지는 프로그램 아래에도 있어야 합니다(Design Studio에 상주하거나 Design Studio에 복제할 수 없음).
...
[필드 추가](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/addFieldToAFormUsingPOST) 에는 상위 양식의 id와 필드의 fieldId만 필요합니다. 다른 모든 필드는 비어 있거나 데이터 형식 및 필드 메타데이터를 기반으로 하는 기본값을 가집니다. 데이터는 JSON이 아닌 POST x-www-form-urlencoded로 전달됩니다.
...
위의 예에서는 간단한 문자열인 LastName 필드를 업데이트합니다. 일부 양식 필드는 더 복잡합니다. 예를 들어 인사말 필드는 항목 목록과 기본값을 포함하는 &quot;select&quot; 필드 유형입니다. 선택 항목 중 하나를 선택으로 설정하지 않은 한, 선택 유형 필드를 추가하거나 업데이트하는 경우 `isDefault` 값이 true인 경우 첫 번째 선택에는 값이 없고 &quot;선택...&quot;이라는 레이블이 지정됩니다.
...
양식의 필드는 를 통해 모두 단일 단위로 재배열해야 합니다. [양식 필드 위치 변경](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/updateFieldPositionsUsingPOST) 엔드포인트. 끝점에 라는 매개 변수가 필요합니다. `positions`: 3개의 멤버가 있는 JSON 오브젝트 배열입니다.
...
서식 있는 텍스트 필드는 [개별 엔드포인트](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/addRichTextFieldUsingPOST) 리드 필드에서 가져왔습니다. 필드 콘텐츠는 다중 파트/양식 데이터로 전달됩니다. 스크립트, 메타 태그 또는 링크 태그를 포함하지 않는 HTML 컨텐츠로 구조화해야 합니다.
...
필드 세트 내의 필드는 전체적으로 양식에 고유하므로 중복 필드가 양식의 상위 필드 목록과 하위 필드 세트 모두에 있지 않을 수 있습니다. 필드 세트는 를 통해 추가됩니다. [양식에 필드 세트 추가](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/addFieldSetUsingPOST) endpoint 및 가 다음 결과에 나타납니다. [양식에 대한 필드 가져오기](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/getFormFieldByFormVidUsingGET). 필드는 를 통해 필드 세트의 fieldList로 이동하여 필드 세트에 추가됩니다. [필드 위치 업데이트](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/updateFieldPositionsUsingPOST). 이러한 끝점의 경우 데이터는 JSON이 아닌 POST x-www-form-urlencoded로 전달됩니다.
...
사용 가능한 연산자의 전체 목록이 필요하면 끝점 참조 페이지를 참조하십시오. [양식 필드 가시성 규칙 추가](https://developer.adobe.com/marketo-apis/api/asset/#tag/Form-Fields/operation/addFormFieldVisibilityRuleUsingPOST).

## 후속 작업

Marketo forms에는 제출 시 지정된 필드의 내용에 따라 지정된 페이지로 리디렉션하거나 현재 페이지에 머무르는 규칙이 적용되는 동적 후속 페이지 동작이 있을 수 있습니다. 규칙은 서로 바꿔서 감사 페이지 규칙 또는 후속 페이지 규칙이라고 할 수 있습니다. 이러한 규칙은 멤버와 함께 JSON 배열로 표시됩니다 `followupType`, `followupValue`, `operator`, `subjectField`, `values`, 및 `default`. `default` 는 배열에 있는 하나의 레코드만 true일 수 있는 부울 값입니다. 방문자가 다른 규칙을 사용할 수 없는 경우 기본값으로 지정된 규칙이 사용됩니다. `followupType` lp 또는 url일 수 있습니다. 여기서 lp는 Marketo 랜딩 페이지 id를 나타냅니다. `followupValue`및 url은 다른 페이지에 대한 URL을 나타냅니다. 연산자는 제공된 값 목록과 주제 필드의 값을 비교하는 데 사용됩니다.

## 전송 단추

양식의 제출 단추 스타일은 [업데이트 제출 단추](https://developer.adobe.com/marketo-apis/api/asset/#tag/Forms/operation/updateFormSubmitButtonUsingPOST) 엔드포인트. buttonPosition, buttonStyle, label 및 waitingLabel(제출이 보류 중일 때 표시되는 레이블)은 수정할 수 있습니다.
