...
비동기적으로 실행되는 요청을 제출하면 데이터가 수집됩니다. 에서 이벤트를 구독하여 요청 상태를 검색할 수 있습니다. [Marketo Observability 데이터 스트림](https://developer.adobe.com/events/docs/guides/using/marketo/marketo-observability-data-stream-setup/). &#x200B;

인터페이스는 사용자, 사용자 정의 객체, 이렇게 두 가지 객체 유형에 대해 제공됩니다. 레코드 작업은 &quot;삽입 또는 업데이트&quot;만 가능합니다.

데이터 수집 API는 비공개 베타에 있습니다. 초대받은 사람에게는 다음에 대한 권한이 있어야 합니다. [Marketo Engage 성능 계층 패키지](https://nation.marketo.com/t5/product-documents/marketo-engage-performance-tiers/ta-p/328835).

## 인증

데이터 수집 API는 Marketo REST API와 동일한 OAuth 2.0 인증 방법을 사용하여 액세스 토큰을 생성하지만, HTTP 헤더를 통해 액세스 토큰을 전달해야 합니다 `X-Mkto-User-Token`. 쿼리 매개 변수를 통해 액세스 토큰을 전달할 수 없습니다.
...
도메인 이름은 다음과 같습니다. `mkto-ingestion-api.adobe.io`

경로는 다음으로 시작합니다. `/subscriptions/_MunchkinId_` 위치 `_MunchkinId_` 는 Marketo 인스턴스에만 적용됩니다. Munchkin ID는 Marketo Engage UI의 **관리자** >**내 계정** > **지원 정보**. 경로의 나머지 부분은 관심 리소스를 지정하는 데 사용됩니다.
...
모든 응답은 다음을 통해 고유 요청 ID를 반환합니다. `X-Request-Id` 머리글입니다.
...
호출에서 오류가 발생하면 추가 오류 세부 정보가 있는 응답 본문과 함께 비202 상태가 반환됩니다. 응답 본문은 application/json이며 멤버가 있는 단일 개체를 포함합니다 `error_code` 및 `message`.
...
| 데이터 중복 제거 필드 | 오브젝트 | 아니요 | 중복을 제거할 속성입니다. 하나 또는 두 개의 속성 이름이 허용됩니다. AND 작업에는 두 가지 속성이 사용됩니다. 예를 들어, 두 가지 모두 `email` 및 `firstName` 지정된 경우 둘 다 AND 작업을 사용하여 개인을 조회하는 데 사용됩니다. 지원되는 속성은 다음과 같습니다.`idemail`, `sfdcAccountId`, `sfdcContactId`, `sfdcLeadId`, `sfdcLeadOwnerIdCustom` attributes (&quot;string&quot; 및 &quot;integer&quot; type 전용) | 이메일 |
...
개인에 대한 링크 필드가 요청에 지정되어 있고 해당 개인이 존재하지 않는 경우 몇 번의 재시도가 발생합니다. 재시도 기간(65분) 동안 해당 개인이 추가되면 성공적으로 갱신됩니다. 예를 들어 링크 필드가 `email` 개인에 있고 개인이 존재하지 않는 경우 재시도가 발생합니다.
...
- 인증하려면 다음을 사용하여 액세스 토큰을 전달해야 합니다. `X-Mkto-User-Token` 머리글
- URL 도메인 이름은 다음과 같습니다. `mkto-ingestion-api.adobe.io`
- URL 경로는 다음으로 시작합니다. `/subscriptions/_MunchkinId_`
- 쿼리 매개 변수가 없습니다.
- 호출이 성공하면 202 상태가 반환되고 응답 본문이 비어 있습니다
- 호출이 실패하면 202가 아닌 상태가 반환되고 응답 본문에 가 포함됩니다. `{ "error_code" : "_Error Code_", "message" : "_Message_" }`
- 요청 ID는 를 통해 반환됩니다. `X-Request-Id` 머리글
