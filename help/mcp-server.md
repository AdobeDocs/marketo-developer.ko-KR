...
* Blender와 같은 3D 모델링 앱에 연결하여 애니메이션 빌드
* 비디오 편집을 위해 After Effects에 연결

MCP는 통신 프로토콜로, 모든 애플리케이션이 AI 도구에 데이터와 작업을 노출하기 위해 구현할 수 있는 개방형 표준입니다.

## [!DNL Marketo] MCP가 수행하는 작업과 수행하지 않는 작업
...
* OAuth 토큰 새로 고침을 자동으로 처리 — 토큰 만료를 관리할 필요가 없습니다.
* 테넌트가 격리된 환경 내에서 작동하여 데이터가 다른 사용자의 세션과 교차하지 않도록 합니다

**MCP:**

* AI 또는 머신 러닝 모델을 사용, 호스팅 또는 실행합니다. 모든 AI 처리는 MCP가 아닌 AI 도구에서 수행됩니다.
* 고객 데이터를 포함한 모든 데이터를 교육하거나 학습합니다.
* 예측, 권장 사항 또는 의사 결정 생성 — 의사 결정은 다운스트림 AI 툴 또는 사용자의 책임입니다.
...
>[!TIP]
>
>여러 [!DNL Marketo] 인스턴스에 연결하려면 MCP 구성에 고유한 이름(예: `marketo-prod` 및 `marketo-staging`)을 가진 개별 항목을 해당 자격 증명으로 각각 추가하십시오.
