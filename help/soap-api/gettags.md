...
이 함수는 Marketo에 설정된 비채널 태그에 대한 다음 정보를 검색합니다.

- 태그 유형
- 태그 값

모든 비채널 태그에 대한 정보를 가져오거나 특정 태그 및/또는 값 목록을 지정할 수 있습니다.

## 요청

| 필드 이름 | 필수/선택 사항 | 설명 |
| --- | --- | --- |
| tagList->tag | 선택 사항입니다 | 쿼리할 태그 목록입니다. 각 태그는 문자열로 유형 및 값 배열을 가질 수 있습니다. `<tagList>`이(가) 생략되면 모든 채널에 대한 정보를 받게 됩니다. |
