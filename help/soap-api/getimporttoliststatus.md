...
feature: SOAP
description: getImportToListStatus를 사용하여 Marketo 정적 목록 가져오기 상태, 필수 필드, XML 요청 및 응답, PHP 및 Java 코드, 폴링 모범 사례를 확인하십시오.
...
이 함수는 특정 `importToList` 함수 호출의 상태를 확인합니다.

먼저 `importToList` API 호출을 완료한 다음 `getImportToListStatus`을(를) 정기적으로 폴링합니다. `getImportToListStatus`을(를) 분당 두 번 이상 폴링하지 마십시오.
...
| listName | 필수 | 가져오기 상태를 검색할 Marketo의 정적 목록 이름입니다. |
