...
이 함수는 Marketo에 설정된 채널에 대한 다음 정보를 검색합니다.

- 진행 상태
- 각 진행 상태의 단계 번호
- 특정 진행 상태가 성공으로 정의되었는지 여부

모든 채널에 대한 정보를 가져오거나 특정 채널 목록을 지정할 수 있습니다

## 요청

| 필드 이름 | 필수/선택 사항 | 설명 |
| --- | --- | --- |
| tag->values->stringItem | 선택 사항입니다 | 쿼리할 채널 목록입니다. `<tag>`이(가) 생략되면 모든 채널에 대한 정보를 받게 됩니다. |
